...
from nn import Generator, NeuralNetwork
...
class TrainableGenerator(Generator):
    def __init__(self, field_structure: dp.FieldStructure, column_structure: dp.ColumnStructure, config: ApplicationConfiguration):
        ...
        self.unknown_column_structure = dp.ColumnStructure.make(self.unknown_field_structure, self.config.dtype, dp.PORTION_UNKNOWN - dp.PORTION_WIN)
    ...
    def make_summaries(self, predictions: tf.Tensor, ground_truth: tf.Tensor, main_accuracy: tf.Tensor, loss: Optional[tf.Tensor] = None):
        ...
        for field in self.field_structure.unknown_without_win:
            ...
            column_ground_truth = ground_truth[:, column_slice]
            if field.handling == dp.FieldSpecification.HANDLING_UNARY:
    ...
    def make_output(self, logit: tf.Tensor) -> tf.Tensor:
        ...
        for data_slice, handling in self.unknown_column_structure.generate_handling_slices(self.config.ignored_columns):
            ...
            elif handling == dp.FieldSpecification.HANDLING_ONEHOT or handling == dp.FieldSpecification.HANDLING_BOOL or handling == dp.FieldSpecification.HANDLING_UNARY:
        ...
        def generate_ndarray_batches():
            ...
            for batch in np.split(ndarray, np.arange(self.config.batch_size, ndarray.shape[0], self.config.batch_size), axis=0):
                ...
                if isinstance(x, np.memmap) or not x.flags.writeable:
                    ...
                    x_new[:, :] = x
                    x = x_new
                x = data.columns.randomly_unspecify_optional_columns(x, self.random_state, 0.5, 0.5)
                yield (x, y)
    ...
    def train(self, file_name: str, data: dp.DataProvider):
        ...
        with tf.Session(config=config) as session:
            ...
            while True:
                ...
                if do_train_eval:
                    samples_since_train_evaluation = 0
                    tf_writer.add_summary(train_result['summary'], samples)
                if do_test_eval:
                    samples_since_test_evaluation = 0
                    ...
                    tf_writer.add_summary(test_result['summary'], samples)
...
class TrainableWinEstimator(NeuralNetwork):
    def __init__(self, field_structure: dp.FieldStructure, column_structure: dp.ColumnStructure, config: ApplicationConfiguration):
        super().__init__(field_structure, column_structure, config)
        ...
        self.logger.log(logging.DEBUG, "Initialize layers for trainable win estimator")
        self.layers = self.make_layers(len(column_structure.known_indices) + len(column_structure.unknown_without_win_indices), 1)
    ...
    def make_layers(self, n_input: int, n_output: int) -> List[Tuple[tf.Variable, tf.Variable]]:
        ...
        total_parameter_amount = 0
        previous_layer_size = n_input
        for i, l in enumerate(itertools.chain(self.config.w_hidden_layer_structure, (n_output,))):
            self.logger.log(logging.DEBUG, "Initialize weights with size {x:d}×{y:d}={s:d}".format(x=previous_layer_size, y=l, s=previous_layer_size*l))
            total_parameter_amount += previous_layer_size * l
            w = tf.Variable(tf.random_normal((previous_layer_size, l), 0, 0.05, dtype=self.config.dtype, seed=self.config.seed), name="weights_" + str(i))
            b = tf.Variable(tf.random_normal((l, 1), 0, 0.05, dtype=self.config.dtype, seed=self.config.seed), name="bias_" + str(i))
    ...
    def make_minimizer(self, loss: tf.Tensor) -> Tuple[tf.Operation, tf.Variable]:
        step = tf.Variable(0, trainable=False, name='win_estimator_step')
        # grads_and_vars is a list of tuples (gradient, variable)
        grads_and_vars = self.config.w_optimizer(self.config.w_learning_rate).compute_gradients(loss, var_list=self.parameters)
        capped_grads_and_vars = [(tf.clip_by_value(gv[0], -5., 5.), gv[1]) for gv in grads_and_vars]
        # apply the capped gradients.
        minimizer = self.config.w_optimizer(self.config.w_learning_rate).apply_gradients(capped_grads_and_vars, global_step=step)
        return minimizer, step

    def make_summaries(self, predictions: tf.Tensor, ground_truth: tf.Tensor, accuracy: tf.Tensor):
        train_summaries = tf.summary.merge([
            tf.summary.scalar("win_train_accuracy", tf.reduce_mean(accuracy)),
            tf.summary.scalar("win_train_error", tf.reduce_mean(tf.math.abs(predictions - ground_truth))),
        ])
        test_summary_list = [
            tf.summary.scalar("win_test_accuracy", tf.reduce_mean(accuracy)),
            tf.summary.scalar("win_test_error", tf.reduce_mean(tf.math.abs(predictions - ground_truth))),
            tf.summary.histogram("win_predictions", predictions),
        ]
        for i, (w, b) in enumerate(self.layers):
            test_summary_list.append(tf.summary.histogram("win_weights_{:d}".format(i), w))
            test_summary_list.append(tf.summary.histogram("win_biases_{:d}".format(i), b))
        test_summaries = tf.summary.merge(test_summary_list)
        return train_summaries, test_summaries
    ...
    def make_train_ndarray_batch_generator(self, data: dp.DataProvider):
        def generate_ndarray_batches():
            ...
            for batch in np.split(ndarray, np.arange(self.config.batch_size, ndarray.shape[0], self.config.batch_size), axis=0):
                batch = data.columns.shuffle_participants(batch, self.random_state)
                x: np.ndarray = batch[:, data.columns.known_slice]
                w: np.ndarray = batch[:, [data.columns.win_indices[0]]]
                # take some optional information from x out (simulate incomplete user input)
                if isinstance(x, np.memmap) or not x.flags.writeable:
                    x_new = np.ndarray(x.shape, x.dtype)
                    x_new[:, :] = x
    ...
    def make_test_ndarray_generator(self, data: dp.DataProvider):
        def generate_ndarray():
            ndarray = data.get_ndarray()[-self.config.test_data_amount - self.config.validation_data_amount:-self.config.test_data_amount, :]
            x = ndarray[:, data.columns.known_slice]
            w = ndarray[:, [data.columns.win_indices[0]]]
            # take some optional information from x out (simulate incomplete user input)
            if isinstance(x, np.memmap) or not x.flags.writeable:
                x_new = np.ndarray(x.shape, x.dtype)
                x_new[:, :] = x
    ...
    def train(self, file_name: str, data: dp.DataProvider, generator: TrainableGenerator):
        train_iterator, test_iterator, iterator_handle, data_iterator = self.make_data_iterator(data)
        known, win = data_iterator.get_next(name="layer_in")
        unknown_logit = generator.make_logit(known)
        unknown = generator.make_output(unknown_logit)
        win_logit = self.make_logit(tf.concat((known, unknown), axis=1))
        loss, accuracies = self.make_metrics(win_logit, win)
        accuracy = tf.reduce_mean(accuracies)
        win_predictions = self.make_output(win_logit)
        train_summary, test_summary = self.make_summaries(win_predictions, win, accuracy)
        minimizer, step = self.make_minimizer(loss)
        config = tf.ConfigProto()
        config.gpu_options.allow_growth = True
        with tf.Session(config=config) as session:
            ...
            while True:
                ...
                self.logger.log(logging.DEBUG, "Do one training step...")
                train_results = session.run(run_parameters, feed_dict={iterator_handle: train_handle})
                train_result = dict(item for d in train_results for item in d.items())

                if do_train_eval:
                    samples_since_train_evaluation = max(0, samples_since_train_evaluation - self.config.samples_per_train_evaluation)
                    tf_writer.add_summary(train_result['summary'], samples)
                if do_test_eval:
                    samples_since_test_evaluation = max(0, samples_since_test_evaluation - self.config.samples_per_test_evaluation)
                    test_result = session.run({
                        'summary': test_summary,
                        'loss': loss,
                        'accuracy': accuracy,
